...
class Epins extends Component {
    constructor(props) {
        ...
        handleClick(event) {
          ...
          axios.post(globalVariables.user_api_path+'/epin/search', {model_call: 'Epin', search_f:'epin_id', offset: this.state.ActivePage, user_id: user_id}, {
        ...
        componentDidMount() {
          let login_token = sessionStorage.getItem('login_token');
          let user_id = sessionStorage.getItem('user_id');
          axios.post(globalVariables.user_api_path+'/epin/search', {model_call: 'Epin', search_f:'epin_id', user_id: user_id}, {
            headers: { Authorization: "Bearer " + login_token }
          })
            .then(res => res.data).then((data) => {
              console.log(data);
            ...
            axios.post(globalVariables.user_api_path+'/epin/search-packages', {model_call: 'Epin_package', search_f:'package_name'}, {
              headers: { Authorization: "Bearer " + login_token }
            })
              .then(res => res.data).then((data) => {
                let packageList = [];
        ...
        handleSubmit = event => {
          ...
          let login_token = sessionStorage.getItem('login_token');
          axios.post(globalVariables.user_api_path+'/epin/generate', {no_of_epin: this.state.no_of_epin, amount: this.state.amount, expiry_date: this.state.expiry_date},{
            headers: { Authorization: "Bearer " + login_token }
          })
            .then(res => {
              this.props.handleClick("tr", 1, "Epin Generated Successfully");
              axios.post(globalVariables.user_api_path+'/epin/search',  {model_call: 'Epin', search_f:'epin_id'},{
                  headers: { Authorization: "Bearer " + login_token }
                })
                  .then(res => res.data).then((data) => {
                    this.setState({contentList: data.response});
    ...
    return (
      <div className="main-content">
        <Grid fluid>
          <Row>
            <Col md={12}>
              <Card title={<span>E-Pin List </span>}
                tableFullWidth
                content={
                  <div>
                  <Table responsive>
